...
plotQDNAseq <- function(set, path.to.profile, subtitle=NULL){
	...
	for (i in 1:length(sampleNames(set))) {
		...
		plot(set[,i], dotres=1, ylim=c(-3,5), ylab=expression(normalized~log[2]~read~count), main=paste(sampleNames(set)[i], subtitle[i], sep=" "),
