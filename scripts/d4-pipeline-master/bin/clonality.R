...
if((clonTab$cor > 0.54 & clonTab$llr2 > 0) & Non_clonal_prob > 0.5){
    outlier <- T
    main <- 'This sample is an outlier and likely non-clonal'
}else if((clonTab$llr2 < -5 | clonTab$cor < 0.45) & Non_clonal_prob < 0.5){
    outlier <- T
    main <- 'This sample is an outlier and likely clonal'
}else{
    outlier <- F
    main = ''
}
...
points(GoldStandard_Data$llr2,GoldStandard_Data$cor,  pch=20, cex=1, col = ifelse(as.integer(as.factor(GoldStandard_Data$clonality)) == 2, rgb(0.44, 0.16, 0, 0.15),rgb(0, 0, 0.25, 0.15)))
